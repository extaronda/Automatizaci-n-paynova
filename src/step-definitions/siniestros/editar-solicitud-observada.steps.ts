...
Given('que estoy en la bandeja de solicitudes de SINIESTROS', async function() {
  const editarPage = new EditarSolicitudPage(global.page);
  await editarPage.navegarABandeja();
  console.log('✓ Navegado a la bandeja de solicitudes de SINIESTROS');
});

// ==================== ACCIONES ====================

When('busco una solicitud de SINIESTROS con "Paso Actual" igual a "OBSERVADO"', async function() {
  const editarPage = new EditarSolicitudPage(global.page);
  await editarPage.buscarSolicitudObservadaYClicOjo();
});

When('hago clic en el botón del ojo para ver el detalle de SINIESTROS', async function() {
...
When(/^hago clic en el botón "EDITAR SOLICITUD" en SINIESTROS$/, async function() {
  const editarPage = new EditarSolicitudPage(global.page);
  await editarPage.clicEditarSolicitud();
});

When('modifico el monto a {string} en SINIESTROS', async function(nuevoMonto: string) {
  const editarPage = new EditarSolicitudPage(global.page);
  await editarPage.modificarMonto(nuevoMonto);
});

When(/^hago clic en el botón "ACTUALIZAR" en SINIESTROS$/, async function() {
  const editarPage = new EditarSolicitudPage(global.page);
  await editarPage.clicActualizar();
});

When(/^hago clic en el botón "ENVIAR" en SINIESTROS$/, async function() {
  const editarPage = new EditarSolicitudPage(global.page);
...
Then('verifico que el campo "Paso Actual" sea "OBSERVADO" en SINIESTROS', async function() {
  const editarPage = new EditarSolicitudPage(global.page);
  const esObservado = await editarPage.verificarPasoActualObservado();
  expect(esObservado).toBeTruthy();
});

Then('verifico que el monto se actualiza en la grilla de datos guardados de SINIESTROS', async function() {
  const editarPage = new EditarSolicitudPage(global.page);
...
Then('la solicitud de SINIESTROS es enviada exitosamente', async function() {
  ...
  if (procesoCompletado) {
    console.log('   ✓ La solicitud de SINIESTROS fue enviada y procesada exitosamente');

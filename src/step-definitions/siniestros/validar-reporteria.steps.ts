...
When('accedo a la página de Reportería de Solicitudes de SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  await reporteriaPage.navegarAReporteria();
  console.log('   ✓ Navegado a Reportería de Solicitudes de SINIESTROS');
});
...
function cargarPlantillaValidacion(): any {
  ...
  if (!plantillaCache) {
    ...
    const plantillaPath = path.join(process.cwd(), 'test-data', 'plantilla-validacion-reporteria.json');
...
Then('debería ver los filtros disponibles en Reportería de SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  const plantilla = cargarPlantillaValidacion();
  
  console.log('\n   📋 Validando Filtros de Reportería de SINIESTROS (usando plantilla JSON):');
...
Then('debería ver la tabla de Reportería con las columnas correctas en SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  const plantilla = cargarPlantillaValidacion();
  
  console.log('\n   📊 Validando Columnas de Tabla Reportería de SINIESTROS (usando plantilla JSON):');
...
When('filtro por estado {string} en SINIESTROS', async function(estado: string) {
  const reporteriaPage = new ReporteriaPage(global.page);
  await reporteriaPage.seleccionarEstado(estado);
  this.estadoSeleccionado = estado;
});

When('presiono el botón Consultar en SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  await reporteriaPage.presionarBotonConsultar();
});

Then('debería ver solo las solicitudes con estado APROBADO en Reportería de SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
...
Then('debería ver solo las solicitudes con estado PENDIENTE_APROBACION en Reportería de SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  console.log('   ✅ Todas las solicitudes de SINIESTROS tienen estado PENDIENTE_APROBACION');
});

Then(/^cada registro APROBADO debe tener Asiento con valor válido \(no vacío ni '-'\) en SINIESTROS$/, async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  console.log(`   ✅ Todos los ${registrosAprobados.length} registros APROBADO de SINIESTROS tienen Asiento válido`);
});

Then('cada registro PENDIENTE_APROBACION debe tener Asiento igual a {string} en SINIESTROS', async function(valorEsperado: string) {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  console.log(`   ✅ Todos los ${registrosPendientes.length} registros PENDIENTE_APROBACION de SINIESTROS tienen Asiento igual a "${valorEsperado}"`);
});

Then('debería validar las columnas obligatorias para estado APROBADO en SINIESTROS:', async function(dataTable: DataTable) {
  const reporteriaPage = new ReporteriaPage(global.page);
  const plantilla = cargarPlantillaValidacion();
  ...
  expect(registros.length).toBeGreaterThan(0);
  console.log(`\n   📊 Validando ${registros.length} registro(s) APROBADO de SINIESTROS (usando plantilla JSON):`);
  ...
  console.log('   ✅ Columnas obligatorias de APROBADO de SINIESTROS validadas correctamente');
});

Then('debería validar las columnas obligatorias para estado PENDIENTE_APROBACION en SINIESTROS:', async function(dataTable: DataTable) {
  const reporteriaPage = new ReporteriaPage(global.page);
  const plantilla = cargarPlantillaValidacion();
  ...
  expect(registros.length).toBeGreaterThan(0);
  console.log(`\n   📊 Validando ${registros.length} registro(s) PENDIENTE_APROBACION de SINIESTROS (usando plantilla JSON):`);
  console.log(`      Columnas obligatorias: ${columnasObligatorias.join(', ')}`);
  ...
  console.log('   ✅ Columnas obligatorias de PENDIENTE_APROBACION de SINIESTROS validadas correctamente');
});

When('busco una solicitud por correlativo desde solicitudes-creadas.json en Reportería de SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  console.log(`   ✓ Buscando solicitud de SINIESTROS: ${solicitud.correlativo} (filtro aplicado automáticamente)`);
});

Then('debería ver los resultados filtrados por correlativo en Reportería de SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
...
When('selecciono la moneda {string} en el filtro de Moneda en SINIESTROS', async function(moneda: string) {
  const reporteriaPage = new ReporteriaPage(global.page);
...
Then('debería ver que el filtro de moneda se aplicó correctamente en SINIESTROS', async function() {
  // Solo validar que el filtro se aplicó (el filtro es reactivo y se aplica automáticamente)
  console.log('   ✅ Filtro de moneda de SINIESTROS aplicado correctamente');
});

When('ingreso un incidente en el filtro de Incidente en SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  if (!solicitud || !solicitud.incidente) {
    throw new Error('No se encontraron solicitudes de SINIESTROS con incidente en solicitudes-creadas.json');
  }
  ...
  console.log(`   ✓ Filtrando por incidente de SINIESTROS: ${solicitud.incidente} (filtro aplicado automáticamente)`);
});

When('ingreso el incidente {string} en el filtro de Incidente en SINIESTROS', async function(incidente: string) {
  const reporteriaPage = new ReporteriaPage(global.page);
  // NOTA: El filtro de Incidente es reactivo, se aplica automáticamente (no requiere CONSULTAR)
  await reporteriaPage.filtrarPorIncidente(incidente);
  this.incidenteBuscado = incidente;
  console.log(`   ✓ Filtrando por incidente de SINIESTROS: ${incidente} (filtro aplicado automáticamente)`);
});

Then('debería ver los resultados filtrados por incidente en SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  console.log(`   ✅ Resultados filtrados correctamente por incidente de SINIESTROS: ${incidente}`);
});

Then('debería ver los resultados filtrados por incidente en Reportería de SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  const incidente = this.incidenteBuscado;
  ...
  console.log(`   ✅ Resultados filtrados correctamente por incidente en Reportería de SINIESTROS: ${incidente}`);
});

When('hago clic en el botón Exportar Excel en Reportería de SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  this.nombreArchivoDescargado = await reporteriaPage.exportarAExcel();
});

Then('debería descargarse el archivo Excel correctamente de Reportería de SINIESTROS', async function() {
  const nombreArchivo = this.nombreArchivoDescargado;
  
  expect(nombreArchivo).toBeTruthy();
  expect(nombreArchivo).toContain('.xlsx');
  
  console.log(`   ✅ Archivo Excel de Reportería de SINIESTROS descargado correctamente: ${nombreArchivo}`);
});

// ==================== NUEVOS ESCENARIOS DE FILTROS ====================

When('selecciono el memo {string} en el filtro de Memo en SINIESTROS', async function(memo: string) {
  const reporteriaPage = new ReporteriaPage(global.page);
  await reporteriaPage.seleccionarMemo(memo);
  this.memoSeleccionado = memo;
});

Then('debería ver solo las solicitudes con memo {string} en SINIESTROS', async function(memoEsperado: string) {
  const reporteriaPage = new ReporteriaPage(global.page);
  
  const todasConMemo = await reporteriaPage.verificarMemoEnResultados(memoEsperado);
  expect(todasConMemo).toBeTruthy();
  
  console.log(`   ✅ Todas las solicitudes de SINIESTROS tienen memo "${memoEsperado}"`);
});

When('selecciono fecha inicio {string} en el filtro de SINIESTROS', async function(fecha: string) {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  console.log(`   ✓ Fecha inicio seleccionada en SINIESTROS: ${fecha}`);
});

When('selecciono fecha fin {string} en el filtro de SINIESTROS', async function(fecha: string) {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  console.log(`   ✓ Fecha fin seleccionada en SINIESTROS: ${fecha}`);
});

Then('debería ver solo las solicitudes dentro del rango de fechas seleccionado en SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  console.log(`   ✅ Resultados filtrados correctamente por rango de fechas de SINIESTROS: ${this.fechaInicio} a ${this.fechaFin}`);
});

When('presiono el botón Limpiar Filtros en SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  await reporteriaPage.presionarBotonLimpiar();
});

Then('los filtros deberían estar vacíos en SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  
  const filtrosVacios = await reporteriaPage.verificarFiltrosVacios();
  expect(filtrosVacios).toBeTruthy();
  
  console.log('   ✅ Filtros de SINIESTROS están vacíos');
});

Then('debería ver todas las solicitudes sin filtros aplicados en SINIESTROS', async function() {
  const reporteriaPage = new ReporteriaPage(global.page);
  ...
  console.log(`   ✅ Se muestran todas las solicitudes de SINIESTROS sin filtros: ${registros.length} registros`);
});
